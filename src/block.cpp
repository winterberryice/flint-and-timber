...
namespace flint {
    Block::Block(BlockType type) : block_type(type), tree_id(std::nullopt), sky_light(0), block_light(0) {
    }

    Block Block::new_with_tree_id(BlockType block_type, uint32_t tree_id) {
        Block block(block_type);
        block.tree_id = tree_id;
        return block;
    }
    ...
    std::array<std::array<float, 2>, 6> Block::get_texture_atlas_indices() const {
        switch (block_type) {
            ...
            default:
                // Same as Air, a recognizable "error" texture
                return {{{15.0f, 15.0f}, {15.0f, 15.0f}, {15.0f, 15.0f}, {15.0f, 15.0f}, {15.0f, 15.0f}, {15.0f, 15.0f}}};
        }
    }
}
