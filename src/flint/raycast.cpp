...
namespace flint
{
        ...
        float max_distance)
    {
        ...
        float yaw_radians = glm::radians(player.get_yaw());
        float pitch_radians = glm::radians(player.get_pitch());
        ...
        glm::ivec3 current_voxel_coord(
            std::floor(eye_position.x),
            std::floor(eye_position.y),
            std::floor(eye_position.z));
        ...
        while (current_distance < max_distance)
        {
            ...
            Block block = world.get_block_at_world(current_voxel_coord.x, current_voxel_coord.y, current_voxel_coord.z);
            if (block.is_solid())
            {
